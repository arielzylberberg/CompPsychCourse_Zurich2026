function [err,P] = wrapper_DTB_parametricbound(theta,D,params)
% function [err,P] = wrapper_DTB_parametricbound(theta,D,params)
...
%
% D: struct with the experimental data: response times, choice, motion
% coherence, and accuracy
%
% params: struct with optional flags and parameters. 
%     If 'pars.notabs_flag = True', the output includes the distribution of non-absorved particles.
%     (mostly useful for variable-duration experiments).
%     'pars.USfunc' specifies the shape of the bound (see the 'expand_bounds.m' function). 
%     'pars.t' times at which to solve the Fokker-Planck equation.
%     'plot_flag': boolean. If true, a plot with data & fit is generated and
%     updated during the optimization.

% OUPUTS
% ~~~~~~~~~~~
% 'err': negative log likelihood of the parameters given the data
% 'P': struct containing the model's distribution of decision times per difficulty
% level, time course of the bounds, proportion of rightward/leftward choices, ...
...
%% save the unabsorbed
if ~isempty(params) && isfield(params,'notabs_flag')
    notabs_flag = params.notabs_flag;
else
    notabs_flag = false;
end
...
%% bounds
if ~isempty(params) && isfield(params,'USfunc')
    USfunc = params.USfunc;
else
    USfunc = 'Exponential';
end
